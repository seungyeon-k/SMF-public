...
def run(config, device, writer, bsave, name):
    ...
    if bsave:
        save_folder = 'figures/figure_2_temp'
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        save_name = os.path.join('figures/figure_2_temp', str(name)) + '.npy'
...
if __name__ == '__main__':
    ...
    for i in [1, 2, 3]:
        for j in [1, 2, 3]:
            for k in [1, 2, 3]:
                name = f'b{i}e{j}c{k}'
                config1 = (f'regularize_config_new_3/regularization_no_reg_shape_{name}', 'regularize_config_new_3.yml', 'model_last.pkl', {})
                config2 = (f'regularize_config_new_3/regularization_reg_shape_{name}', 'regularize_config_new_3.yml', 'model_last.pkl', {})
                device = f'cuda:{0}'

                logdir_home = 'train_results/figure_2_temp'
                ...
                run(config, device, writer, bsave, name)